...
function doInBatches(callbackFunction, {collection, startFromId, batchSize = 1000, limit = Infinity, doneCount = 0, query = {}, batchCallBack, message, inSequence = false, firstCall = true, live = false, estimatedCount, progressBar, reportProgress = true}) {
  if (message) {
    console.log(message)
  }
    ...
    getRecordBatch({collection, startFromId, batchSize, query})
      ...
      .then(result => {
        if (reportProgress) {
          ...
          reportUpdate({doneCount, estimatedCount, progressBar})
        }
        if (doneCount < limit) {
          resolve(doInBatches(callbackFunction, {
            collection,
...
function reportUpdate({doneCount, estimatedCount, progressBar}) {
  if (progressBar) {
...
function stopProgressBar(progressBar) {
  if(progressBar && typeof progressBar.stop === 'function') {
    progressBar.stop()
  }
}
