...
function printStats({languageStats, locationStats, tweetStats}) {
  ...
  printLanguageStats(languageStats, tweetStats)
}

function printLanguageStats(languageStats, tweetStats) {
  const strStats = []
  const limit = 15
  let added = 0
  for(let i = 0; i < languageStats.length && added < limit; i++) {
    ...
    if(name) {
      const percentage = languageCount.count * 100 / tweetStats.totalSize
