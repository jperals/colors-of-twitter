...
function saveGeojson(geoJson) {
  return Promise.all([
    jsonfile.writeFile('./output/language-areas.json', geoJson),
    jsonfile.writeFile(getFileNameWithDate(), geoJson)
  ])
}

function getFileNameWithDate() {
  const now = new Date()
  const timestamp = now.getFullYear() + '-' + leftPad((now.getMonth() + 1), 2, '0') + '-' + leftPad(now.getDate(), 2, '0') + '-' + now.getTime()
  return path.join(__dirname, '../output/language-areas-' + timestamp + '.png')
}
